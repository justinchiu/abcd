...
def prepare_dataloader(tok, answ_tok, args):
    ...
    for split in ["train", "val", "test"]:
        ...
        everything = prepare_abcd(
            tok,
            answ_tok,
            split,
            fixed=args.truncate_paragraph,
            max_e=args.max_e_len,
        )
        dataset = AbcdDataset(everything)
        shuffle = True if split == "train" else False
        bs = args.batch_size if split == "train" else args.eval_batch_size
        dataloader = DataLoader(
            dataset, shuffle=shuffle, collate_fn=data_collator, batch_size=bs
        )
...
def run_answer_model(model, input_ids, attn_mask, answs, tokenizer, train):
    ...
    if train:
        outputs = model(
            input_ids=input_ids, attention_mask=attn_mask, labels=answs
        ).loss
...
def run_model(
    batch, model, linear, answer_model, tokenizer, answer_tokenizer, train=True
):
    for key in batch:
    ...
    souts = run_sent_model(
        linear, tokenizer, batch["input_ids"], lm_outs, batch["num_s"], batch["s_maps"]
    )
    ...
    answ_out = run_answer_model(
        answer_model, answer_in, answer_attn, labels, answer_tokenizer, train=train
    )
...
def evaluate(steps, args, model, linear, answ_model, tok, answ_tok, dataloader, split):
    ...
    for step, eval_batch in enumerate(dataloader):
        eval_outs, sent_outs, _ = run_model(
            eval_batch, model, linear, answ_model, tok, answ_tok, train=False
        )
        raw_sent_preds = []
        sent_preds = []
    ...
    if args.save_results and split == "Valid":
        torch.save(
            (sent_results, gold_sents, answ_results, gold_answ),
            f"logging/unsupervised|{args.run_name}|step-{steps}.pt",
        )
...
def main():
    ...
    optim, lr_scheduler = prepare_optim_and_scheduler(
        [model, linear, answer_model], args
    )
    ...
    for epoch in range(args.epoch):
        for step, batch in enumerate(train_dataloader):
            if (
                ...
                answer_model.train()
            _, _, loss = run_model(
                batch, model, linear, answer_model, tokenizer, answer_tokenizer
            )
            loss.backward()
            if (
